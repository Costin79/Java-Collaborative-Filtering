...
public class ItemsAdjBaseline {
	
	private List<PredictedRating> predictedSet;
	private DataSample sample;
	
	public ItemsAdjBaseline(DataSample sample){
	...
	private double calculateRating(String itemId, String userId, ItemsAdjSimMatrix similarityMatrix, 
	...
	private double predictRating(String userId, String itemId, List<CommonBusiness>list, 
		...
		double dividend = list.stream()
								.map(b->(b.getSimilarity()*(b.getStars()-(average+(userAverages.get(userId)-average)+(itemAverages.get(b.getBusId())-average)))))
